...
Q_sim=read.csv(file="out/HERA_Val4_19502020.csv")
HERA_cordata<-read.csv(file="out/HERA_CorStatv3_19512020.csv")

replax=match(as.numeric(HERA_cordata[1,]),as.numeric(Q_sim[1,]))[-1]
Q_sim[,replax]=HERA_cordata[,-1]

date2=seq(as.Date("1951-01-01"),as.Date("2020-12-31"),by="days")
...
sd=which(Q_sim[1,]==Station)
ziz=Q_sim[-1,sd]
plot(ziz)
ziz[c((start_time-20) : (end_time+4))]
...
for (id in 1:length(Station_data_IDs)){
  ...
  lR=c(lR,l)
  sy=min(year(time2[which(!is.na(Qs))]))
  syR=c(syR,sy)
  ey=max(year(time2[which(!is.na(Qs))]))
  eyR=c(eyR,ey)
}
plot(lR[order(lR)])
Station_ID= as.numeric(Q_obs_val[1,-1])
RecordLen=data.frame(Station_ID,lR,syR,eyR)
RecordLen=RecordLen[order(RecordLen$syR,RecordLen$eyR),]
RecordLen$id=c(1:length(syR))

#already keep the suited stations here

# Create the ggplot
ggplot(RecordLen, aes(xmin = syR, xmax = eyR, y = id)) +
  geom_segment(aes(x = syR, xend = eyR, y = id, yend = id, col=lR/365.25), size = .1,alpha=0.4) +
  labs(x = "Year", y = "Station", title = "Length of Record for Each Station") +
  geom_vline(xintercept=1955,col="red",lwd=1)+
geom_vline(xintercept=2015,col="red",lwd=1)+
  theme_minimal()

RecordLen$keep=0
RecordLen$keep[which(RecordLen$syR<=1955 & RecordLen$eyR>=2015)]=1

length(which(RecordLen$keep==1))
RecordKeep=RecordLen[which(RecordLen$keep==1),]
...
for (s in 1:length(Station_data_IDs)){
  ...
  qH10=quantile(HERA_loc,.15,na.rm=T)
  qO10=quantile(Q_loc,.15,na.rm=T)
  ...
  r_hq=cor(HERA_loc[which(Q_loc>qO95)],Q_loc[which(Q_loc>qO95)],use="complete.obs")
  r_lq=cor(HERA_loc[which(Q_loc<=qO10)],Q_loc[which(Q_loc<=qO10)],use="complete.obs")
  ex_ev=match_extremes(sim=HERA_loc,obs=Q_loc,quantile=0.95,tail="high")
...
kgefile="out/EFAS_obs_kgeAY4.csv"
...
length(which(kge_h1$nse>0.5))/length(kge_h1$V1)
plot(ptn$skill-ptn$V2)
hist(kge_h1$nse,xlim=c(-1,1),breaks=2000)
median(kge_h2$V2)
...
if (metric =="kge"){
  ...
  ggsave("Plots/NSE_H2.jpg", map,width=18, height=20, units=c("cm"),dpi=1000)
...
TsEvaNs<- function(timeAndSeries, timeWindow, transfType='trendPeaks',minPeakDistanceInDays=10,
                   ...
                   tail="low", epy=-1, lowdt=7, trans=NULL, TrendTh=NA){
  ...
  if (tail=="low"){
    ...
    start_index=1
    indices_to_extract <- seq(from = start_index, to = length(series), by = 7/dt)
  ...
  if (transfType == 'trend'){
    ...
    if (is.na(TrendTh)){
      ...
      trasfData = tsEvaTransformSeriesToStationaryPeakTrend1( timeStamps, series, timeWindow, TrendTh);
      ...
      if (TrendTh=="MMX"){
        ...
        timeb=timeStamps
        serieb <- series
        timeb[which(serieb < qd)] <- NA
        checkY=check_timeserie2(timeb,bounds)
        while(checkY==FALSE){
          TrendTh=TrendTh-0.05
          print(TrendTh)
          qd <- quantile(series, TrendTh, na.rm = T)
          serieb <- series
          timeb=timeStamps
        ...
        while ((length(unique(trasfData$trendSeries))<2)) {
          trasfData = tsEvaTransformSeriesToStationaryPeakTrend1( timeStamps, series, timeWindow, TrendTh=TrendTh-c);
          c=c+0.1
        }
      }
    }
...
tsGetPOT <- function(ms, pcts, desiredEventsPerYear,minEventsPerYear, minPeakDistanceInDays, tail) {
  ...
  pkx <- declustpeaks(data = ms[,2] ,minpeakdistance = minPeakDistance ,minrundistance = minRunDistance, qt=thrsdts[1])
...
tsEvaTransformSeriesToStationaryPeakTrend1<-function (timeStamps, series, timeWindow, TrendTh) 
{
  ...
  serieb <- series
  serieb[which(serieb < qd)] <- NA
  rs <- tsEvaDetrendTimeSeries1(timeStamps, serieb, timeWindow,TrendTh)
  detrendSeries <- series - rs@trendSeries
  detrendSerie1 <- serieb - rs@trendSeries

  qd2 <- min(detrendSerie1, na.rm = T)
  nRunMn <- rs@nRunMn
  minNThreshold=(1-TrendTh)*nRunMn
  varianceSeries <- tsEvaNanRunningVariance1(detrendSerie1, 
                                            nRunMn,minNThreshold/4)
  varianceSeries <- tsEvaNanRunningMean1(varianceSeries, ceiling(nRunMn/2),minNThreshold/8)
  #plot(varianceSeries)
  stdDevSeries1 <- varianceSeries^0.5
  stdDevSeries <- stdDevSeries1
  avgStdDev <- mean(stdDevSeries,na.rm=T)
  S <- 2
  N <- nRunMn
...
tsEvaDetrendTimeSeries1<-function (timeStamps, series, timeWindow, TrendTh,percent = NA, fast = TRUE) 
{
  extremeLowThreshold <- -Inf
  trendSeries <- tsEvaRunningMeanTrend1(timeStamps, series,
                                       timeWindow,TrendTh)
  ...
  if (tdim == "hours") 
    dtx <- dt/24
  if (fast == FALSE) 
    message(paste0("timestep: ", dt, tdim, " | time window: ", 
                   round(trendSeries$nRunMn * dtx/365.25), " years"))
  statSeries[which(statSeries < extremeLowThreshold)] <- NA
  if (!is.na(percent)) {
    message(paste0("trend on the ", percent, " percentile"))
    trendSeries_perc <- tsEvaNanRunningPercentiles(timeStamps, 
                                                   series, trendSeries$nRunMn, percent)
    meanTrperc <- mean(trendSeries_perc$rnprcnt)
    trendSeriesxp <- trendSeries_perc$rnprcnt/meanTrperc * 
    mean(trendSeries$trendSeries)
  ...
  return(new("tsTrend", originSeries = statSeries, detrendSeries = detrendSeries, 
             trendSeries = trendSused, nRunMn = trendSeries$nRunMn))
}

tsEvaNanRunningVariance1<-function (series, windowSize, minNThreshold) 
{
...
tsEvaRunningMeanTrend1 <- function(timeStamps, series, timeWindow,TrendTh) {
  dt1 <- min(diff(timeStamps), na.rm = T)
  dt <- as.numeric(dt1)
  tdim <- attributes(dt1)$units
  if (tdim == "hours") 
  ...
  trendSeries1 <- tsEvaNanRunningMean1(series, nRunMn,minNThreshold = minNThreshold/4)
  #further smoothing
  trendSeries <- tsEvaNanRunningMean1(trendSeries1, ceiling(nRunMn/2),minNThreshold = 1)
  #plot(trendSeries)

  return(list(trendSeries = trendSeries, nRunMn = nRunMn))
}
tsEvaNanRunningMean1<-function (series, windowSize, minNThreshold) 
{
  rnmn <- matrix(nrow = length(series), ncol = 1)
...
interid<- function(data,trans,WindowSize) {
  ...
  if (length(which(is.na(data$Q7)))==length(data$Q7)){
    ...
    if (length(which(is.na(data$Qs)))>0){
      print("Na alert")
      seriefill=tsEvaFillSeries1(data$date,data$Qs)
      data$Qs=seriefill
    }
    #tinversing the discharge for EVA
    #Identification of the intermittent river
    ...
    m0=length(which(data$Qs7==mindis))
    start_index=1
    #keep 7 day frequency of data
    indices_to_extract <- seq(from = start_index, to = length(data$Q7), by = 7/dt)
...
tsEvaFillSeries1<- function(timeStamps, series){
...
sk=match(RecordKeep$Station_ID,ValidSYl_out$V1)
ValidSYl_outL=ValidSYl_out[sk,]
...
ThDir<-paste0(workDir,"TrendAnalysis/trenTH2")
THX=c()
for (Nsq in 1:88){
...
for (dset in c("obs","sim")){
  ...
  print(dset)
  print(ldt)
  for (s in 1:length(ValidSYl_outL$outl2)){
    id_HERA=s
    # ValidSYl_out$outl2[s]
    Station=ValidSYl_outL$V1[s]
    Station="6213800"
    s=which(Station_data_IDs==Station)
    print(s)
    ...
    uparea=ValidSYl_outL$upa[which(ValidSYl_outL$V1==Station)]
    woo=ValidSYl_outL[which(ValidSYl_outL$V1==Station),]
    ...
    txx=time2
    start_time <- Sys.time()
    catch=as.numeric(ValidSYl_outL$outl2[s])
    timeStamps=txx
    ...
    if(length(na.omit(series$dis))>1 & interflag<3 & nv>15){
      ...
      if (length(which(is.na(series$dis)))>0){
        print("Na alert")
        seriefill=tsEvaFillSeries1(series$timestamp,series$dis)
      ...
      Nonstat<-TsEvaNs(timeAndSeries, timeWindow, transfType='trendPeaks',ciPercentile = ciPercentile, 
                       minPeakDistanceInDays = minPeakDistanceInDays,lowdt=ldt,trans=trans,tail = tail,
                       TrendTh = thresh)
      nonStationaryEvaParams=Nonstat[[1]]
      stationaryTransformData=Nonstat[[2]]
      
      nonStationaryEvaParams$potObj$parameters
      stationaryTransformData$timeStampsDay=unique(as.Date(stationaryTransformData$timeStamps))
      #plot(stationaryTransformData$timeStampsDay, stationaryTransformData$trendSeries)
    ...
    bf=which(colnames(RLgpd)==2015)
    NaDtector=length(which(is.na(RLgpd[bf])))
    if (NaDtector>0){
      print("NAAAAAAAA")
      plot(stationaryTransformData$timeStampsDay, nonStationaryEvaParams$potObj$parameters$threshold)
      plot(stationaryTransformData$timeStampsDay,stationaryTransformData$nonStatSeries)
      abline(v=as.Date("2015-12-31"))
      RLgpd[bf]=-9999
      Sys.sleep(5)
    }
  ...
  save(Results_sim, file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_",ldt,".Rdata"))
  }
  if (dset=="obs"){
    Results_obs=list(parameters=parlist,RetLevGEV=RetLevGEV,RetLevGPD=RetLevGPD,RetPerGEV=RetPerGEV,RetPerGPD=RetPerGPD,Peaks=peaklist,catrest=data.frame(catlist,IRES))
    save(Results_obs, file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_",ldt,".Rdata"))
  }
...
tail="low"
dset="obs"
load(file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_7.Rdata"))
dset="sim"
load(file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_7.Rdata"))


#load(file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_31.Rdata"))
#Results_obs=Results_sim
dset="sim"
haz="flood"
tail="high"
load(file=paste0(hydroDir,"/TrendAnalysis/ResValid_",haz,"_",dset,"_1951_2020_HERA2_7.Rdata"))
...
#Step one: understand the distribution of Nan
Parameter=as.data.frame(Results_obs_d$parameters)

#loop to know when my parameters start and stop
eyX=c()
syX=c()
for (ix in unique(Parameter$catchment)){
  parix=Parameter[which(Parameter$catchment==ix),]
  sy=min(parix$Year[which(!is.na(parix$thresholdGPD))])
  ey=max(parix$Year[which(!is.na(parix$thresholdGPD))])
  syX=c(syX,sy)
  eyX=c(eyX,ey)
}

SimLen=data.frame(unique(Parameter$catchment),syX,eyX)
SimLen=SimLen[order(SimLen$syX,SimLen$eyX),]
SimLen$id=c(1:length(syX))

#already keep the suited stations here

# Create the ggplot
ggplot(SimLen, aes(xmin = syX, xmax = eyX, y = id)) +
  geom_segment(aes(x = syX, xend = eyX, y = id, yend = id), size = .1,alpha=0.4) +
  labs(x = "Year", y = "Station", title = "Length of Record for Each Station") +
  geom_vline(xintercept=1955,col="red",lwd=1)+
  geom_vline(xintercept=2015,col="red",lwd=1)+
  theme_minimal()
...
#Joining both modelled and observed length
mabg=(match(SimLen$unique.Parameter.catchment.,ValidSYl_out$outl2))
SimLen$Station_ID=ValidSYl_out$V1[mabg]
SimRecord=inner_join(RecordLen,SimLen,by=c("Station_ID"))

#keep only stations that have enough data points
SimRecordKeep=SimRecord[which(SimRecord$keep==1),]


#If I am in drought mode I have to put a minus sign
if (haz=="drought"){
  #Remove station with IR
  SimRecordKeep=SimRecordKeep[-which(is.infinite(SimRecordKeep$syX)),]
  IRES_obs=Results_obs$catrest
  IRES_sim=Results_sim$catrest
  IRES_sim$IRobs=IRES_obs$IRES

  IR_locs=which(IRES_obs$IRES==3 | IRES_sim$IRES==3)
...
sk=match(RecordKeep$Station_ID,ValidSYl_out$V1)
ValidSYl_outL=ValidSYl_out[sk,]
#keep only long term stations
sk=match(SimRecordKeep$Station_ID,ValidSYl_outL$V1)
ValidSYl_outL=ValidSYl_outL[sk,]


#RLGPD_obs=RLGPD_obs[-IR_locs,]
RLGPD_obs=data.frame(outl=Results_obs$catrest$catlist,IRES=IRES_sim$IRES+IRES_obs$IRES ,RLGPD_obs)
RLGPD_sim=data.frame(outl=Results_sim$catrest$catlist,IRES=IRES_sim$IRES+IRES_obs$IRES ,RLGPD_sim)

#remove all probelmatic pixels
removed=unique(c(which(abs(RLGPD_obs$X2015)==9999),which(abs(RLGPD_sim$X2015)==9999),
                 which(is.nan(RLGPD_obs$X2015)),which(is.nan(RLGPD_obs$X1955)),
                 which(is.nan(RLGPD_sim$X1955))))
RLGPD_obs=RLGPD_obs[-removed,]
RLGPD_sim=RLGPD_sim[-removed,]
cn=colnames(RLGPD_obs)
yb=which(cn=="X1955")
RLGPD_trend=(RLGPD_obs[,-c(1,2)]-RLGPD_obs[,yb])
RLGPD_trendsim=(RLGPD_sim[,-c(1,2)]-RLGPD_sim[,yb])
yi=which(cn=="2015")

if (haz=="drought"){
  mdata=RLGPD_obs[-which(RLGPD_obs$IRES>1),c(1,2)]
  RLGPD_trend=RLGPD_trend[-which(RLGPD_obs$IRES>1),]
  RLGPD_trendsim=RLGPD_trendsim[-which(RLGPD_sim$IRES>1),]
  plot(as.numeric(RLGPD_trend[10,]))
  
  RLGPD_obs2=RLGPD_obs[-which(RLGPD_obs$IRES>1),]
  RLGPD_sim2=RLGPD_sim[-which(RLGPD_sim$IRES>1),]
  cor(RLGPD_obs2$X1955,RLGPD_sim2$X1955)
  
} else{
  mdata=RLGPD_obs[,c(1,2)]
  RLGPD_obs2=RLGPD_obs
  RLGPD_sim2=RLGPD_sim
}


coco=c()
coca=c()
for (id in 1:length(RLGPD_trend[,1])){
  co=cor(as.numeric(RLGPD_trend[id,]),as.numeric(RLGPD_trendsim[id,]),method = "spearman",use="complete.obs")
  coco=c(coco,co)
  
  ca=cor(diff(as.numeric(RLGPD_trend[id,])),diff(as.numeric(RLGPD_trendsim[id,])),method = "pearson",use="complete.obs")
  coca=c(coca,ca)
}
...
mean(coco,na.rm=T)
mean(coca)
length(which(coco>0))/length(coco)
hist(coca)
cococa=data.frame(mdata,coco,coca)
#first plot: histogram of correlation between observed and modelled change
limi=c(-1,1)
name=( expression(paste("Spearman's ", rho)))
p<-ggplot(cococa, aes(x=coco)) + 
  geom_histogram(fill="aliceblue", color="grey20",alpha=0.9,lwd=1,bins=20) +
  geom_point(x=median(cococa$coco,na.rm=T),y=0,pch=21,size=4,stroke=2,fill="royalblue")+
...
p
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/HCor_1955_2015_dr7.jpg", p,width=18, height=20, units=c("cm"),dpi=1000)


#map of change correlation
Cplot=inner_join(ValidSYl_outL,cococa,by=c("outl2"="outl"))
Corplot <- st_as_sf(Cplot, coords = c("Var1.x", "Var2.x"), crs = 4326)
Corplot <- st_transform(Corplot, crs = 3035)
palet=c(hcl.colors(11, palette = "RdYlBu", alpha = NULL, rev = F, fixup = TRUE))

library(ggnewscale)
map=ggplot(basemap) +
  geom_sf(fill="gray95", color=NA) +
  geom_sf(data=Corplot,aes(geometry=geometry,fill=coco,size=upa),color="transparent",alpha=.9,shape=21,stroke=0)+ 
  scale_fill_gradientn(
    colors=palet,n.breaks=6,oob = scales::squish, limits=c(-1,1),name="r") +
  #guides(fill = guide_colourbar(barwidth = 15, barheight = 1,reverse=F),size="none")+
  geom_sf(data=Corplot,aes(geometry=geometry,size=upa),col="grey20",alpha=1,stroke=0.05,shape=1)+ 
  geom_sf(fill=NA, color="grey20") +
  scale_x_continuous(breaks=seq(-30,40, by=5)) +
  scale_size(range = c(1, 3), trans="sqrt")+
  coord_sf(xlim = c(min(nco[,1]),max(nco[,1])), ylim = c(min(nco[,2]),max(nco[,2])))+
  ...
  theme(axis.title=element_text(size=tsize),
        ...
        # legend.key = element_rect(fill = "transparent", colour = "transparent"),
        legend.key.size = unit(.8, "cm"))+
  ggtitle(paste0("Correlation between observed and modelled change in 10-y ",haz," RL"))

map
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/trend_1955_2015_cor_dr7.jpg", map,width=18, height=20, units=c("cm"),dpi=1000)

save(Corplot,file="D:/tilloal/Documents/LFRuns_utils/data/TrendAnalysis/correlation_trend_floods.Rdata")

cn=colnames(RLGPD_sim)
RLGPD_trend=data.frame(mdata ,RLGPD_trend)
cn=colnames(RLGPD_trend)
yb=which(cn=="X1955")
yi=which(cn=="X2015")


RLGPD_trend=inner_join(ValidSYl_outL,RLGPD_trend[,c(1,2,yi)],by=c("outl2"="outl"))
RLGPD_obsS=inner_join(ValidSYl_outL,RLGPD_obs2[,c(1,yi,yb)],by=c("outl2"="outl"))
RLGPD_obsS$X1955_qsp=RLGPD_obsS$X1955/RLGPD_obsS$upa*1000
RLGPD_obsS$X2015_qsp=RLGPD_obsS$X2015/RLGPD_obsS$upa*1000

RLGPD_trendsim=data.frame(mdata,RLGPD_trendsim)
cn=colnames(RLGPD_trendsim)

RLGPD_trendsim=inner_join(ValidSYl_outL,RLGPD_trendsim[,c(1,yi)],by=c("outl2"="outl"))
RLGPD_simS=inner_join(ValidSYl_outL,RLGPD_sim2[,c(1,yi,yb)],by=c("outl2"="outl"))
RLGPD_simS$X1955_qsp=RLGPD_simS$X1955/RLGPD_simS$upa*1000
RLGPD_simS$X2015_qsp=RLGPD_simS$X2015/RLGPD_simS$upa*1000
plot(((RLGPD_obsS$X2015_qsp)-(RLGPD_obsS$X1955_qsp))/((RLGPD_obsS$X1955_qsp+RLGPD_obsS$X2015_qsp)/2))
# points(RLGPD_obs$X1955,RLGPD_sim$X1955,col=2)
# abline(a=0,b=1)

a=as.numeric((RLGPD_sim2$X2015-RLGPD_sim2$X1955))
b=as.numeric((RLGPD_obs2$X2015-RLGPD_obs2$X1955))
cor(a,b,use="complete.obs")
plot(RLGPD_trend$X2015,RLGPD_trendsim$X2015)
cor(RLGPD_trend$X2015,RLGPD_trendsim$X2015,use="complete.obs")

Recaplus <- st_as_sf(RLGPD_trend, coords = c("Var1.x", "Var2.x"), crs = 4326)
Recaplus <- st_transform(Recaplus, crs = 3035)
names(Recaplus)[23]="trend"
...
library(matrixStats)
mobs=rowMins(as.matrix(RLGPD_obs2[,c(3:72)]),na.rm=T)
mom=rowMaxs(as.matrix(RLGPD_obs2[,c(3:72)]),na.rm=T)
vic=mom/mobs
vic[which(is.infinite(vic))]=max(vic[-which(is.infinite(vic))])
plot(sqrt(vic[order(vic)]))
epso=(log(vic))/2
epso=rep(.1,length(vic))
# epso=epso/Recaplus$upa*1000
plot(epso)
length(which(vic>1))
# epso[which(vic>1)]=.5
# epso[which(vic>1.5)]=1
# epso[which(vic>2)]=2
# epso[which(vic>5)]=5
# epso[which(vic>10)]=50
# plot(vic,epso)
RLGPD_obs2$eps=epso

msim=rowMins(as.matrix(RLGPD_sim2[,c(3:72)]),na.rm=T)
msm=rowMaxs(as.matrix(RLGPD_sim2[,c(3:72)]),na.rm=T)
vic=msm/msim
epss=rep(.1,length(vic))
#epss=epss/Recaplus$upa*1000
plot(epss)
vic[which(is.infinite(vic))]=max(vic[-which(is.infinite(vic))])
plot(sqrt(vic[order(vic)]))
#epss=log((vic))/2

plot(epss[order(epss)])
median(epss)
length(which(vic>2))
# epss[which(vic>1)]=.2
# epss[which(vic>1.5)]=1
# epss[which(vic>2)]=2
# epss[which(vic>5)]=5
# epss[which(vic>10)]=50
RLGPD_sim2$eps=epss
...
for (st in 1:length(Recaplus$outl2)){
  ...
  mat=match(sta,RLGPD_sim2$outl)
  wallou=as.numeric(RLGPD_sim2[mat,-c(1,2,73)])
  uph=Recaplus$upa[st]
  tschange=wallou/uph*1000
  #plot(tschange)
  eps=RLGPD_sim2$eps[mat]
  qspI=RLGPD_simS$X1955_qsp[st]
  qspF=RLGPD_simS$X2015_qsp[st]
  trend_qsp1=Recaplus$trendsim_qsp[st] /((qspI+qspF)/2+eps)*100
  #if (trend_qsp1<2) break
  trend_qsp=c(trend_qsp,trend_qsp1)
  
  eps=RLGPD_obs2$eps[mat]
  print(eps)
  qspI=RLGPD_obsS$X1955_qsp[st]
  qspF=RLGPD_obsS$X2015_qsp[st]
  trend_qsp2=Recaplus$trend_qsp[st]/((qspI+qspF)/2+eps)*100
  #if (trend_qsp2>1000) break
  trend_qop=c(trend_qop,trend_qsp2)
  
}

plot(Recaplus$trendsim_qsp)
plot(trend_qop)
mean(RLGPD_obsS$X1955_qsp)
plot(trend_qop,trend_qsp)

cor.test(Recaplus$trend_qsp,Recaplus$trendsim_qsp,method = c( "spearman"))
cor.test(trend_qop,trend_qsp,method = c( "pearson"))

cor.test(Recaplus$trend,Recaput$trend,method = c( "kendall"))


Recaplus$trend_oqv=trend_qop
Recaplus$trend_sqv=trend_qsp
...
map=ggplot(basemap) +
  geom_sf(fill="gray95", color=NA) +
  geom_sf(data=Recaplus,aes(geometry=geometry,fill=trend_sqv,size=upa),color="transparent",alpha=.9,shape=21,stroke=0)+ 
...
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/trend_sim2_1955_2015_relative_dr.jpg", map,width=18, height=20, units=c("cm"),dpi=1000)
...
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/trend_difference_1955_2015_relativ_fl2.jpg", map,width=18, height=20, units=c("cm"),dpi=1000)
...
ps<-ggplot(data=Recapsame, aes(x=trend_oqv, y=trend_sqv, size=upa), color="black") +
  ...
  annotate("label", x=-40, y=60, label= paste0("r = ",round(sqrt(R2lf),2),"\nAgreement = ",truesign,"%"),size=5)+
  ...
  scale_x_continuous(name="Observed symetric change (%)",breaks = c(-vlim,-150,-100,-50,-25,-10,0,10,25,50,100,150,vlim),limits=c(-vlim,vlim),
                     expand = c(0, 0), trans=scales::modulus_trans(.6))+
  scale_y_continuous(name="Simulated symetric change (%)",breaks =  c(-vlim,-150,-100,-50,-25,-10,0,10,25,50,100,150,vlim),limits=c(-vlim,vlim),
                     expand = c(0, 0),trans=scales::modulus_trans(.6))+
  ...
  theme(axis.title=element_text(size=tsize),
        ...
        legend.key = element_rect(fill = "transparent", colour = "transparent"),
        legend.key.size = unit(.8, "cm"))+
  ggtitle(paste0("HERA - Observed and simulated trends of\n10-y RL ",haz," (1955-2015) (n = ",lt," stations)"))# adjust transparency range if needed

ps

ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/scatter2_correlation_HERAvsobs_19552015_Htseva27.jpg", ps, width=20, height=20, units=c("cm"),dpi=300)



##### Locations where both trends are well reproduced -------------------

load("D:/tilloal/Documents/LFRuns_utils/data/TrendAnalysis/correlation_trend_floods.Rdata")
corflood=Corplot
load("D:/tilloal/Documents/LFRuns_utils/data/TrendAnalysis/correlation_trend_droughts7.Rdata")
cordrought=Corplot

corail=st_join(corflood,cordrought,by="V1")

corail$meanc=(corail$coco.y+corail$coco.x)/2


plot(corail$coco.y,corail$coco.x)
corail$minc=pmin(corail$coco.y,corail$coco.x)

corail=corail[-which(is.na(corail$coco.y)),]

ro=expression(paste(rho))

hist(corail$minc)
map=ggplot(basemap) +
  geom_sf(fill="gray95", color=NA) +
  geom_sf(data=corail,aes(geometry=geometry,fill=meanc,size=upa.x),color="transparent",alpha=.9,shape=21,stroke=0)+ 
  scale_fill_gradientn(
    colors=palet,n.breaks=6,oob = scales::squish, limits=c(-1,1),name=ro) +
  #guides(fill = guide_colourbar(barwidth = 15, barheight = 1,reverse=F),size="none")+
  geom_sf(data=corail,aes(geometry=geometry,size=upa.x),col="grey20",alpha=1,stroke=0.05,shape=1)+ 
  geom_sf(fill=NA, color="grey20") +
  scale_x_continuous(breaks=seq(-30,40, by=5)) +
  scale_size(range = c(1, 3), trans="sqrt")+
  coord_sf(xlim = c(min(nco[,1]),max(nco[,1])), ylim = c(min(nco[,2]),max(nco[,2])))+
  ...
  theme(axis.title=element_text(size=tsize),
        ...
        # legend.key = element_rect(fill = "transparent", colour = "transparent"),
        legend.key.size = unit(.8, "cm"))+
  ggtitle(paste0("mean(flood,drought) correlation between observed and modelled change in 10-y RL"))

map
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/trend_1955_2015_cor_fldr_mean.jpg", map,width=18, height=20, units=c("cm"),dpi=1000)



#by upstream area
corail$UpA=corail$upa.x
corail$UpAgroup=1
corail$UpAgroup[which(corail$UpA>200 & corail$UpA<=500)]=2
corail$UpAgroup[which(corail$UpA>500 & corail$UpA<=1000)]=3
corail$UpAgroup[which(corail$UpA>1000 & corail$UpA<=10000)]=4
corail$UpAgroup[which(corail$UpA>10000 & corail$UpA<=100000)]=5
corail$UpAgroup[which(corail$UpA>100000)]=6


agUpA=aggregate(list(upav=corail$UpA),
                by = list(upa=corail$UpAgroup),
                FUN = function(x) c(length(x)))

corail$skill=corail$meanc
corail$skill[which(is.na(corail$skill))]=0
median(corail$UpA)
length(which(corail$UpA<1000))/length(corail$V1.x)
meds <- c(by(corail$skill, corail$UpAgroup, median))
q <- c(by(corail$skill, corail$UpAgroup, quantile))
merdecol=match(corail$UpAgroup,names(meds))
corail$col=meds[merdecol]
palet=c(hcl.colors(9, palette = "Blues", alpha = NULL, rev = T, fixup = TRUE))

p1<-ggplot(corail, aes(x=factor(UpAgroup), y=skill)) +
  geom_boxplot(notch=F,position=position_dodge(.9),alpha=.8,aes(fill=col),linewidth=0.8,outlier.alpha = 0.4)+
  scale_y_continuous(limits = c(-1,1),name="Spearman's rho",breaks = seq(-1,1,by=0.5),minor_breaks = seq(-1,1,0.1))+
  scale_x_discrete(labels=c("1" = "100-200", "2" = "200-500",
                            "3" = "500-1000","4" = "1000-10 000",
                            "5" = "10 000-100 000","6" = ">100 000"),name="Catchment Area (km2)")+
  scale_fill_gradientn(
    colors=palet, n.breaks=6,limits=c(0.2,0.99)) +
...
p1
ggsave("D:/tilloal/Documents/LFRuns_utils/TAplots/boxplot_meanrho_fdcor.jpg", p1, width=20, height=15, units=c("cm"),dpi=1500)

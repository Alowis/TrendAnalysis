...
ComputeReturnLevels <- function(nonStationaryEvaParams, RPgoal, timeIndex) {
  ...
  if (nonStationaryEvaParams[[1]]$method == "No fit") {
    ...
    returnLevelsGPD <- tsEvaComputeReturnLevelsGPD(
      epsilonGPD, sigmaGPD, thresholdGPD, epsilonStdErrGPD, sigmaStdErrGPD, thresholdStdErrGPD,
...
processTrendData <- function(trendData, DataTr, id_var = "HydroR") {
  ...
  dftest <- suppressWarnings(melt(dfd,
    id.vars = "Biogeo_id",

...
Catamere07 <- inner_join(hybasf7,
  read.csv(file.path(geoDir, "HYBAS07/from_hybas_eu_onlyid.csv"),
...
# Corrected HER shapefile, stored under the canonical GeoData/HER folder
GHshpp <- read_sf(dsn = file.path(geoDir, "HER/her_all_adjusted_cor.shp"))
...
agg_decade <- function(Trends, drv) {
  ...
  dec <- do.call(
    data.frame,
    aggregate(list(value = tr_dec$value),
      by = list(yr = tr_dec$yr),
      FUN = function(x) {
        c(
          mean = mean(x, na.rm = TRUE),
          med = median(x, na.rm = TRUE),
...
make_sankey <- function(class_list, stage_names, loscolors, ltot) {
  ...
  ggplot(df2, aes(
    x = x.x, next_x = next_x, node = node, next_node = next_node,
    fill = factor(node),
    label = paste0(node, "\n", np, "%")
  )) +
...
base_pix_sf <- st_transform(
  st_as_sf(TotFPix, coords = c("Var1", "Var2"), crs = 4326),
  crs = 3035
)
...
# HER level
databicr <- databiclim
databicr$x <- databiclim$x + databilu$x
databicr$y <- databiclim$y + databilu$y
databicrl <- databiclim
databicrl$x <- databiclim$x + databilu$x + databire$x
databicrl$y <- databiclim$y + databilu$y + databire$y
databicrlw <- databiclim
databicrlw$x <- databiclim$x + databilu$x + databire$x + databiwd$x
databicrlw$y <- databiclim$y + databilu$y + databire$y + databiwd$y



for (obj in c("databicr", "databicrl", "databicrlw")) {
  d <- get(obj)
  d$bi_class <- make_bi_class(d$x, d$y)
  d$combined_category <- d$bi_class
  d$maxicat <- assign_trcat(d$combined_category)
  assign(obj, d)
}
...
databipicr$x <- databipic$x + databipilu$x
databipicr$y <- databipic$y + databipilu$y
databipicrl <- databipic
databipicrl$x <- databipic$x + databipilu$x + databipire$x
databipicrl$y <- databipic$y + databipilu$y + databipire$y
databipicrlw <- databipic
databipicrlw$x <- databipic$x + databipilu$x + databipire$x + databipiwd$x
databipicrlw$y <- databipic$y + databipilu$y + databipire$y + databipiwd$y
...
clixHR <- add_hr(databipic)
lxHR <- add_hr(databipilu)
rexHR <- add_hr(databipire)
wxHR <- add_hr(databipiwd)
totxHR <- add_hr(databipi)
...
mbcli <- agg_both(clixHR)
mblu <- agg_both(lxHR)
mbre <- agg_both(rexHR)
mbwd <- agg_both(wxHR)
mball <- agg_both(totxHR)
...
FDcts <- rbind(
  cbind(FloodAgg$cts, haz = "Flood"),
  cbind(DroughtAgg$cts, haz = "Drought")
)
FDdec <- rbind(
  cbind(FloodAgg$dec, hazard = "Flood", year = seq(1955, 2015, 10)),
...
fig1 <- ggplot(FDcts, aes(x = yr, y = value.mean, color = haz, fill = haz)) +
  ...
  scale_y_continuous(
    name = "Change (% 10yRL)", breaks = br_line,
...
ggsave(paste0(plotDir, "/Fig1_temporalCh_fldr_FINAL.jpg"),
  fig1,
  width = 30, height = 10, units = "cm", dpi = 1000
)
...
map_tot <- ggplot(basemap) +
  geom_sf(fill = "white") +
  geom_sf(
    data = databitot, aes(fill = combined_category, geometry = geometry),
    alpha = 0.7, color = "transparent", size = 0.01, show.legend = FALSE
  ) +
  geom_sf(
    data = databipi, aes(col = bi_class, geometry = geometry, size = upa),
...
fig2 <- ggarrange(map_tot, legend_tot,
  ncol = 2, nrow = 1,
...
map_cli <- ggplot(basemap) +
  geom_sf(fill = "white") +
  geom_sf(
    data = databiclim, aes(fill = combined_category, geometry = geometry),
    alpha = 0.7, color = "transparent", show.legend = FALSE
  ) +
  geom_sf(
    data = databipic, aes(col = bi_class, geometry = geometry, size = upa),
    alpha = 1, stroke = 0, shape = 15, show.legend = FALSE
  ) +
  geom_sf(fill = NA, color = "gray42") +
  scale_fill_manual(values = colorp) +
...
fig3 <- ggarrange(map_cli, legend_tot,
  ncol = 2, nrow = 1,
  widths = c(2, 1), heights = c(1, 1), vjust = -1
)
...
driver_order <- traj_counts %>%
  ...
  arrange(total_pct) %>% # ascending <U+2192> coord_flip puts highest at top
  pull(driver)
...
ggsave(paste0(plotDir, "/Fig5e_FINAL.jpg"),
  fig_bv_bar,
  width = 16, height = 22, units = "cm", dpi = 1000
)
...
ggsave(paste0(plotDir, "/FigS_nona_FINAL.jpg"),
  figNA,
  width = 16, height = 16, units = "cm", dpi = 1000
)
## -- Figure 8 -------------------------------------------------
...
fig8 <- ggplot(basemap) +
  ...
  geom_sf(
    data = bioplot, aes(fill = factor(BR), geometry = geometry),
...
fig9 <- ggplot() +
  ...
  geom_hline(yintercept = 0, color = "black", linewidth = 1.5) +
  geom_point(
    data = mbfH, aes(x = x, y = y, color = biogeo, size = d_val.l),
    stroke = 0, alpha = .7
  ) +
  geom_text(
    data = mbfH, aes(x = x, y = y, label = CODEB),
    size = 3, color = "black", fontface = "bold"
  ) +
  geom_point(
    data = mbfHsig, aes(x = x, y = y, size = d_val.l),
...
if (RUN_SUPP) {
  ...
  figS1 <- ggplot(FDdec) +
    ...
    scale_y_continuous(
      name = "Mean change (% 10yRL)", breaks = br_bxp,
    ...
    scale_fill_manual(values = colorz_haz, name = "Hazard") +
    scale_color_manual(
      values = c("Flood" = "darkblue", "Drought" = "darkorange"),
      name = "Hazard"
    ) +
    theme(
      axis.title = element_text(size = 18, face = "bold"),
  ...
  mbf <- data.frame(
    x = c(
      mean(databiclim$y, na.rm = T), mean(databire$y, na.rm = T),
      mean(databilu$y, na.rm = T), mean(databiwd$y, na.rm = T)
    ),
    y = c(
      mean(databiclim$x, na.rm = T), mean(databire$x, na.rm = T),
      mean(databilu$x, na.rm = T), mean(databiwd$x, na.rm = T)
    ),
    xq1 = c(
      quantile(databiclim$y, .05, na.rm = T), quantile(databire$y, .05, na.rm = T),
      quantile(databilu$y, .05, na.rm = T), quantile(databiwd$y, .05, na.rm = T)
    ),
    xq2 = c(
      quantile(databiclim$y, .95, na.rm = T), quantile(databire$y, .95, na.rm = T),
      quantile(databilu$y, .95, na.rm = T), quantile(databiwd$y, .95, na.rm = T)
    ),
    yq1 = c(
      quantile(databiclim$x, .05, na.rm = T), quantile(databire$x, .05, na.rm = T),
      quantile(databilu$x, .05, na.rm = T), quantile(databiwd$x, .05, na.rm = T)
    ),
    yq2 = c(
      quantile(databiclim$x, .95, na.rm = T), quantile(databire$x, .95, na.rm = T),
  ...
  figS3 <- make_sankey(
    class_list = list(
      databipic$maxicat, databipicr$maxicat,
  ...
  coHR <- sample(
    grDevices::colors()[grep("gr(a|e)y|white|black",
      grDevices::colors(),
      invert = TRUE
    )],
  ...
  figS6 <- ggplot(basemap) +
    geom_sf(fill = "white") +
    geom_sf(
      data = databic2, aes(fill = factor(HR), geometry = geometry),
  ...
  map_se <- ggplot(basemap) +
    geom_sf(fill = "white") +
    geom_sf(
      data = databise, aes(fill = combined_category, geometry = geometry),
      alpha = .7, color = "transparent", show.legend = FALSE
    ) +
    geom_sf(
      data = databipise, aes(col = bi_class, geometry = geometry, size = upa),
      alpha = 1, stroke = 0, shape = 15, show.legend = FALSE
    ) +
    geom_sf(fill = NA, color = "gray42") +
    scale_fill_manual(values = colorp) +
  ...
  figS7 <- ggarrange(map_se, legend_tot,
    ncol = 2, nrow = 1,
    widths = c(2, 1), heights = c(1, 1), vjust = -1
  )
...
trajDir <- file.path(hydroDir, "Trajectories")
if (!dir.exists(trajDir)) dir.create(trajDir, recursive = TRUE, showWarnings = FALSE)

write.csv(mbfH, file = file.path(trajDir, "Histo_res_bvHR_F.csv"), row.names = FALSE)
write.csv(mbfX, file = file.path(trajDir, "Drivers_res_bvHR_F.csv"), row.names = FALSE)
save(databipise, file = file.path(trajDir, "SEchanges_bivariate_F.Rdata"))
save(databipic, file = file.path(trajDir, "CLchanges_bivariate_F.Rdata"))

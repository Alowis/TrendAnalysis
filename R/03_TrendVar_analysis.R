...
# Resolve script directory (works interactively and under Rscript)
...
analyze_hazard_symmetric <- function(param_df,
  ...
  get_col <- function(v, sfx) {
    if (sfx == "") {
      return(if (v == "Sigma") "sigmaGPD" else if (v == "threshold") "thresholdGPD" else v)
    }
    return(paste0(v, sfx))
  }
  ...
  if (analysis_type == "driver") {
    ...
    if (var_type == "threshold") {
      ...
      val_curr <- calcGPDReturnLevel_Single(
        param_df$epsilonGPD, param_df[[get_col("Sigma", curr_sfx)]],
        param_df[[get_col("threshold", curr_sfx)]], param_df$nPeaks,
        70, return_period
      )
      val_base <- calcGPDReturnLevel_Single(
        param_df$epsilonGPD, param_df[[get_col("Sigma", base_sfx)]],
    ...
    if (var_type == "RP") {
      if (haz == "drought") {
        param_df$RP_val <- -calcGPDReturnLevel_Single(
          param_df$epsilonGPD, param_df$sigmaGPD,
  ...
  trendClim <- aggregate(DataV[, yr_cols],
    by = list(HydroR = DataV$HER),
    FUN = function(x) mean(x, na.rm = TRUE)
  ) %>% do.call(data.frame, .)

  col_2015 <- if ("X2015" %in% names(DataV)) "X2015" else "2015"
  pointClim <- aggregate(list(Rchange_rel = DataV[[col_2015]]),
    by = list(HydroR = DataV$HER),
    FUN = function(x) {
...
# Root data directories (hydroDir and plotDir come from config_paths.R)
dataDir <- switch(hazard,
  Flood   = file.path(hydroDir, "Flood/"),
  Drought = file.path(hydroDir, "Drought/")
)
...
if (!exists("outf")) {
  ...
  for (Nsq in 1:88) {
    ...
    # NOTE: 'outlets' uses the 10000-based local index (internal to this
    # script); 'outl2' uses ID_MULT and is the key that joins to the
    # trend/param tables produced by 01/02.
...
df_SCF <- process_hazard_data(
  sub_dir = "SCF", hazard = hazard,
  base_data_dir = dataDir,
  fileVar = "Var_TrendX_agg.Rdata"
)

df_Histo <- process_hazard_data(
  sub_dir = "Histo", hazard = hazard,
  base_data_dir = dataDir,
  fileVar = "Var_TrendX_agg.Rdata"
)

df_WStat <- process_hazard_data(
  sub_dir = "WStat", hazard = hazard,
  base_data_dir = dataDir,
  fileVar = "Var_TrendX_agg.Rdata"
)

df_RWStat <- process_hazard_data(
  sub_dir = "RWStat", hazard = hazard,
  base_data_dir = dataDir,
  fileVar = "Var_TrendX_agg.Rdata"
)
...
message(
  "df_Main ready: ", nrow(df_Main), " rows, ",
...
load(file = file.path(hydroDir, "TrendVar/Flood_pixChange_RL100.Rdata"))
...
for (s_name in names(rl_scenarios)) {
  ...
  out_file <- file.path(hydroDir, hazard, paste0("RL100xx.", out_name, ".Rdata"))

...
id=1
plot(as.numeric(RLGPD_H[id,-71])/as.numeric(RLGPD_SCF[id,-71]),type="l",ylim=c(1e-1,1e1), log="y")
for (id in 2:length(RLGPD_H[,1])){
  print(id)
  lines(as.numeric(RLGPD_H[id,-71])/as.numeric(RLGPD_SCF[id,-71]),col=id)
}

# points(as.numeric(RLGPD_SCF[1300,-71]),col=2)
### flood ----
# load(file=paste0(hydroDir,"/TSEVA_hybas/outputs/parCat07_SocCF_flood_1950_2020.Rdata"))
# load(file=paste0(hydroDir,"/TSEVA_hybas/outputs/ResCat07_SocCF_flood_1950_2020.Rdata"))
...
data_fy=data.frame(Histo=RLGPD_H$X1965,SCF=RLGPD_SCF$X1965,RCF=RLGPD_RCF$X1965, ID=RLGPD_H$unikout)
data_fy$diff=(data_fy$RCF-data_fy$Histo)/data_fy$Histo*100
...
hybmatch=match(data_fy$ID,cst7$outlets)
data_fy$hybasID=cst7$HYBAS_ID[hybmatch]

#catchment plot with difference
data=right_join(catmap,data_fy,by = c("outlets"="ID"))
title=paste0("difference between 1951 10y flood RL in histo and socCF scenarii")
legend="difference (%)"
palet=c(hcl.colors(11, palette = "RdBu", alpha = NULL, rev = F, fixup = TRUE))
...
ggplot(basemap) +
  geom_sf(fill="white")+
  geom_sf(data=data,aes(fill=diff,geometry=geometry),color="transparent")+ 
  geom_sf(fill=NA, color="grey") +
  scale_fill_gradientn(
    colors=palet,
    breaks=br,limits=limi,trans=trans,
...
pid=Peaks_H[which(Peaks_H$catch==id),]
did=datarF[which(datarF$unikout==id),]

did=(as.numeric(did[-c(71,72)]))
#create a time vector with years
dates <- seq.Date(ymd("1951-06-01"), ymd("2020-06-01"), by = "year")
dates= as.POSIXct(dates, format = "%Y-%m-%d")

psid=Peaks_SCF[which(Peaks_SCF$catch==id),]
dsid=RLGPD_SCF[which(RLGPD_SCF$unikout==id),]

dsid=(as.numeric(dsid[-c(71)]))


prid=Peaks_RCF[which(Peaks_RCF$catch==id),]
drid=RLGPD_RCF[which(RLGPD_RCF$unikout==id),]
drid=(as.numeric(drid[-c(71)]))

plot(pid$time,pid$value,ylim=c(200,600),pch=16)
points(psid$time,psid$value,col=2,pch=4)
points(prid$time,prid$value,col=5,pch=1)
lines(dates,did,col=1)
lines(dates,dsid,col=2)
lines(dates,drid,col=5)

#difference attributed to the reservoir(s)
#return level
diffres=did-drid
plot(diffres)
#peaks
mpeaks=inner_join(prid,pid, by="time")
mpeaks=mpeaks[order(mpeaks$time),]
mpeaks$diffpeaks=((mpeaks$value.x-mpeaks$value.y)/mpeaks$value.y)*100
plot(mpeaks$time,mpeaks$diffpeaks,type="o")

#identify the first peak that was influenced by reservoirs. 
tres1=mpeaks$time[which(mpeaks$diffpeaks>1)[1]]


#difference attributed to land use change
#return level
diffres2=drid-dsid
plot(diffres2)
#peaks
mpeaks2=inner_join(psid,prid, by="time")
mpeaks2=mpeaks2[order(mpeaks2$time),]
mpeaks2$diffpeaks=((mpeaks2$value.x-mpeaks2$value.y)/mpeaks2$value.y)*100
plot(mpeaks2$time,mpeaks2$diffpeaks,type="o")
abline(h=-0.5)
abline(h=0.5)
#identify the first peak that was influenced by reservoirs. 
tres2=mpeaks2$time[which(abs(mpeaks2$diffpeaks)>1)[1]]


#work with absolute change?

didi=did/did[1]
dsidi=dsid/dsid[1]
dridi=drid/drid[1]

plot(dates,didi,col=1, type="l")
lines(dates,dsidi,col=2)
lines(dates,dridi,col=5)
abline(v=tres1)
...
int=c(80000:89000)
plot(txx[int],Q_H[int])
points(txx[int],Q_SCF[int],col=2)
...
Nonstat<-TsEvaNs(timeAndSeries, timeWindow, transfType="trendPeaks", minPeakDistanceInDays = minPeakDistanceInDays, tail=tail, trans=trans)
...
Nonstat2<-TsEvaNs(timeAndSeries, timeWindow, transfType="trendPeaks", minPeakDistanceInDays = minPeakDistanceInDays, tail=tail, trans=trans)
nonStationaryEvaParams2=Nonstat2[[1]]
stationaryTransformData2=Nonstat2[[2]]
print(paste0("GPD shape: ",round(nonStationaryEvaParams2$potObj$parameters$epsilon,2)))
print(paste0("GEV shape: ",round(nonStationaryEvaParams2$gevObj$parameters$epsilon,2)))


ExRange= c(min(nonStationaryEvaParams$potObj$parameters$peaks),max(nonStationaryEvaParams$potObj$parameters$peaks))
wr2 <- c(seq(min(ExRange),max(ExRange),length.out=700))

Plot1= tsEvaPlotGPDImageScFromAnalysisObj(wr2, nonStationaryEvaParams2, stationaryTransformData2, minYear = '1951',trans=trans)
Plot1
timeIndex=1

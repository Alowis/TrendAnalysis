...
if (haz == "Drought") namefile="Drought.nonfrost.Histo21"
...
RLGPDflH1=as.data.frame(RLGPDflH1)
RLGPDflH2=as.data.frame(RLGPDflH2)
...
if (haz=="Drought"){
  ...
  load(file=paste0(hydroDir,"/",haz,"/IRES.",season,".Histo2",mmx,".Rdata"))
  IRES_Histo1=IRES_save
  wtfs=IRES_Histo1[which(IRES_Histo1$catlist==8500987),]
  IRES_H1=which(IRES_Histo1$IRES==1)
  load(file=paste0(hydroDir,"/",hazard,"/IRES.",season,".Histo",mmx,".Rdata"))
  IRES_Histo2=IRES_save
  wtfs=IRES_Histo2[which(IRES_Histo2$catlist==8500987),]
  IRES_H2=which(IRES_Histo2$IRES==1)

  #who is missing in SocCF
  
  msoc=match(IRES_Histo1$catlist,IRES_Histo2$catlist)

  #if both scenario have an IR, I can compare the RP value
  IRES_over=inner_join(IRES_Histo1,IRES_Histo2,by="catlist")
  IRES_over$IRES.y[which(is.na(IRES_over$IRES.y))]=0
  IRES_over$IRES.x[which(is.na(IRES_over$IRES.x))]=0
  IRES_over$gen_IR=ceiling((IRES_over$IRES.x+IRES_over$IRES.y)/2)
  IRES_over$dtectblss=(IRES_over$IRES.x+IRES_over$IRES.y)
  names(IRES_over)[c(2,3)]=c("Histo1","Histo2")
  IR_loco=which(IRES_over$dtectblss==2)
  IR_foirad=which(IRES_over$dtectblss==1)
  IRES_dtail=IRES_over[IR_foirad,]
  IRES_dtail$simpl=-1
  IRES_dtail$simpl[which(IRES_dtail$Histo2==1)]=1
  IRES_dstable=IRES_over[IR_loco,]
  IRES_dstable$simpl=0
  IRES_plotz=rbind(IRES_dtail,IRES_dstable)
  
  IRES_comb=full_join(IRES_Histo1,IRES_Histo2,by="catlist")
  IRES_comb$IRES.y[which(is.na(IRES_comb$IRES.y))]=0
  IRES_comb$IRES.x[which(is.na(IRES_comb$IRES.x))]=0
  IRES_comb$gen_IR=ceiling((IRES_comb$IRES.x+IRES_comb$IRES.y)/2)
  IRES_comb$dtectblss=(IRES_comb$IRES.x+IRES_comb$IRES.y)
  names(IRES_comb)[c(2,3)]=c("Histo1","Histo2")
  IR_locs=which(IRES_comb$gen_IR==1)
  
  merdee=IRES_Histo2$catlist[IR_locs]
  match(8500987,merdee)
  merdee[58491]
...
#Plot about Intermittent rivers

RLev1=inner_join(IRES_plotz,UpArea,by=c("catlist"="outl2"))
RLev1 <- st_as_sf(RLev1, coords = c("Var1.x", "Var2.x"), crs = 4326)
RLev1 <- st_transform(RLev1, crs = 3035)
RLev1$modif=RLev1$Y2020+1e-4
...
m1=ggplot(basemap) +
  ...
  geom_sf(data=RLev1,aes(col=factor(simpl),geometry=geometry,size=upa),alpha=1,stroke=0,shape=15)+ 
  ...
  coord_sf(xlim = c(min(nco[,1]),max(nco[,1])), ylim = c(min(nco[,2]),max(nco[,2])))+
  scale_color_manual(
    values=c("-1"="blue","0"="yellow","1"="red"), labels=c("P to IR"," no change","IR to P"),na.value="white",name="Change in intermittent rivers")   +
  labs(x="Longitude", y = "Latitude")+
  # guides(colour = guide_colourbar(barheight = 1.5, barwidth = 16),
...
#m1
ggsave(paste0("D:/tilloal/Documents/LFRuns_utils/TrendAnalysis/plots/RL10_",haz,"_change_IR.jpg"), m1, width=23, height=20, units=c("cm"),dpi=1000) 
...
for (run in 1:2){
  ...
  if (run==1) RLcheck=RLGPDflH1
  if (run==2) RLcheck=RLGPDflH2
  ...
  if (length(pbshit)>1){
    ...
    if (run==1) RLGPDflH1=RLcheck
    if (run==2) RLGPDflH2=RLcheck

  }
}

RLGPDflH1_bkp=RLGPDflH1
RLGPDflH2_bkp=RLGPDflH2

### For drought ----------------

RLGPDflH1=RLGPDflH1_bkp
RLGPDflH2=RLGPDflH2_bkp



if (haz=="Drought"){
  #Histo
  RLIRH1<- RLGPDflH1[IR_loco,]
  #RLGPDflH1[IR_locs,c(1:70)]<-NA
  RLGPDflH1[-IR_loco,c(1:70)]=-RLGPDflH1[-IR_loco,c(1:70)]
  for (id in c(1:70)){
    RLGPDflH1[which(is.infinite(RLGPDflH1[,id])),id]=NA
    RLGPDflH1[which(RLGPDflH1[,id]<0),id]=0
    RLIRH1[which(is.infinite(RLIRH1[,id])),id]=1000
    #RLGPDflH1[which(is.infinite(RLGPDflH1[,id])),id]=1000
  }
  RLGPDflH1[IR_loco,]=RLIRH1
  
  
  RLIRH2<- RLGPDflH2[IR_loco,]
 # RLGPDflH2[IR_locs,c(1:70)]<-NA
  RLGPDflH2[-IR_loco,c(1:70)]=-RLGPDflH2[-IR_loco,c(1:70)]
  for (id in c(1:70)){
    RLGPDflH2[which(is.infinite(RLGPDflH2[,id])),id]=NA
    RLGPDflH2[which(RLGPDflH2[,id]<0),id]=0
    RLIRH2[which(is.infinite(RLIRH2[,id])),id]=1000
   #RLGPDflH2[which(is.infinite(RLGPDflH2[,id])),id]=1000
  }
  RLGPDflH2[IR_loco,]=RLIRH2
  ...
  #         legend.key.size = unit(1, "cm"))
  # 
  # ggsave(paste0("D:/tilloal/Documents/LFRuns_utils/TrendAnalysis/plots/IRES_NoRFGW.jpg"), IRmap, width=20, height=20, units=c("cm"),dpi=1000) 
  # 
  # 
  # 
}
...
Shapepar1=ParamsflH1[,c(1,2,4)]
Shapepar2=ParamsflH2[,c(1,2,4)]

rmfuckers1=which(Shapepar1$epsilonGPD>1)
rmfuckers2=which(Shapepar2$epsilonGPD>1)


rmfuckers=unique(c(rmfuckers1,rmfuckers2))

data = data.frame(RLGPDflH1)
data2 = data.frame(RLGPDflH2)
...
length(which(is.na(RLGPDflH1$Y2020)))
RLev1=inner_join(RLGPDflH1,UpArea,by=c("unikout"="outl2"))
RLev1 <- st_as_sf(RLev1, coords = c("Var1.x", "Var2.x"), crs = 4326)
RLev1 <- st_transform(RLev1, crs = 3035)
RLev1$modif=RLev1$Y2020+1e-4

mierdaaa=which(!is.na((match(RLev1$unikout,RLGPDflH2$unikout))))

miniH2=RLGPDflH2[,c(70,71)]
RLev1=inner_join(RLev1,miniH2,by="unikout")
RLev1$ori=RLev1$Y2020.y+1e-4
RLev1$diff=(RLev1$modif-RLev1$ori)/(RLev1$ori)*100

#RLev1$diff[IR_foirad]=IRES_dtail$simpl
max(RLGPDflH2$Y2020,na.rm=T)
# 
RLev1$dqsp=RLev1$diff/RLev1$upa*1000

max(RLev1$dqsp,na.rm=T)
# min(RLev1$Mqsp,na.rm=T)
# max(RLev1$Mqsp,na.rm=T)

RLverif=RLev1[,c(80:87)]
#remove the crap
...
m1=ggplot(basemap) +
  geom_sf(fill="gray90",color="darkgrey",size=0.5)+
  geom_sf(data=RLev1,aes(col=diff,geometry=geometry,size=upa),alpha=1,stroke=0,shape=15)+ 
  ...
  scale_color_gradientn(
    colors=palet,
    breaks=seq(-100,100,by=25), limits=c(-100,100),
    oob = scales::squish,na.value="purple",name="HERA_NoRF - HERA_original (%)")   +
  labs(x="Longitude", y = "Latitude")+
  guides(colour = guide_colourbar(barheight = 1.5, barwidth = 16),
         fill = guide_colourbar(barheight = 16, barwidth = .6))+
  theme(axis.title=element_text(size=tsize),
...
#m1
ggsave(paste0("D:/tilloal/Documents/LFRuns_utils/TrendAnalysis/plots/RL10_",haz,"_change_NoGWRF_noIR.jpg"), m1, width=23, height=20, units=c("cm"),dpi=1000) 

...
# hydroDir and plotDir come from config_paths.R (canonical data root)
...
osize <- 12
colorz <- c(
  "Clim" = "dodgerblue4", "LUC" = "gold4",
  "Res" = "firebrick4", "WU" = "olivedrab"
)
colorn <- c(
  "WU" = "limegreen", "Res" = "tomato4",
...
aggregate_change <- function(Data, yrname, y_col = "Y2015") {
  pointagg <- aggregate(
    ...
    FUN = function(x) {
      c(
        mean = mean(x, na.rm = TRUE), dev = sd(x, na.rm = TRUE),
        len = length(x),
...
Catchmentrivers7 <- read.csv(
  file.path(geoDir, "HYBAS07/from_hybas_eu_onlyid.csv"),
...
biogeoregions <- raster(file.path(geoDir, "eea_3035_biogeo-regions_2016/Biogeo_rasterized_wsg84.tif"))
...
RLGPDflSCF <- as.data.table(runSCF$RLmat)
ParamsflSCF <- runSCF$Params
PeakSCF <- runSCF$Peaks
...
if (haz == "Drought") {
  ...
  IRES_comb <- Reduce(
    function(a, b) full_join(a, b, by = "catlist"),
...
DataL <- ComputeChange(Soctrend, unikout, DataI, outhybas07,
  rmpixels, UpAvec, GHR_riv, HydroRsf, yrname, "socio",
  eps = 0.1
)
yrange <- match(yrname, colnames(DataL))

res_L <- aggregate_change(DataL, yrname)
pointSoc <- res_L$point
trendSoc <- res_L$trend
...
res_W <- aggregate_change(DataW, yrname)
pointWu <- res_W$point
trendWu <- res_W$trend

## 5.3  Total change -------------------------------------------
DataT <- ComputeChange(Totaltrend, unikout, DataI, outhybas07,
  rmpixels, UpAvec, GHR_riv, HydroRsf, yrname, "total",
  eps = 0.1
)

res_T <- aggregate_change(DataT, yrname)
pointTot <- res_T$point
trendTot <- res_T$trend

## 5.4  Climate ------------------------------------------------
DataC <- ComputeChange(Climtrend, unikout, DataI, outhybas07,
  rmpixels, UpAvec, GHR_riv, HydroRsf, yrname, "clim",
  eps = 0.1
)
res_C <- aggregate_change(DataC, yrname)
pointClim <- res_C$point
trendClim <- res_C$trend
...
res_R <- aggregate_change(DataR, yrname)
pointRes <- res_R$point
trendRes <- res_R$trend
...
trtF_EU <- rbind(
  TdataClim$tGlobal, TdataLuse$tGlobal,
...
fig1 <- ggplot(trtF_EU) +
  ...
  geom_rect(
    aes(
      xmin = year - 4.5, xmax = year + 4.5, ymin = cq1, ymax = cq2,
  ...
  geom_rect(
    aes(
      xmin = year - 4.5, xmax = year + 4.5, ymin = med - 1e-1, ymax = med + 1e-1,
      fill = factor(driver), group = factor(driver)
    ),
    alpha = 1, position = position_dodge(width = 9)
  ) +
  scale_y_continuous(
    name = "Change (% 10-y RL)", breaks = br_bxp,
    trans = scales::modulus_trans(.6)
  ) +
  scale_x_continuous(
    breaks = xlabs, labels = xlabs, name = "Decades",
...
ggsave(paste0(plotDir, "/Fig1_bxp_EU_", haz, "_F2.jpg"),
  fig1,
  width = 30, height = 20, units = "cm", dpi = 400
)
#
...
for (driver in driverlist) {
  ...
  if (haz == "Flood") {
    ...
    if (driver %in% c("climate", "all")) {
      titleX <- paste0(
        "Change in 10-year ", haz, " attributed\nto ", driver,
        " changes (% of 100y flood) — 1955–2015"
      )
      fig_map <- ggplot(basemap) +
        geom_sf(fill = "white", color = "darkgrey", size = 0.5) +
        geom_sf(
          data = pag, aes(fill = Rchange_rel.mean, geometry = geometry),
          alpha = 0.2, color = "transparent"
        ) +
        geom_sf(
          data = points, aes(col = Y2015, geometry = geometry, size = upa),
        ...
        new_scale_fill() +
        geom_sf(
          data = pointsInside, aes(geometry = geometry, fill = sign),
        ...
        guides(
          colour = guide_colourbar(barwidth = 22, barheight = 1),
  ...
  ggsave(paste0(plotDir, "/", fname, "_mapF_", driver, "_", haz, "_F2.jpg"),
    fig_map,
    width = 23, height = 20, units = "cm", dpi = 1000
  )
}
...
titleX_res <- paste0(
  "Change in 10-year ", haz, " attributed to reservoir changes\n(",
...
fig_res <- ggplot(basemap) +
  ...
  geom_sf(
    data = pag_res, aes(fill = Rchange_rel.mean, geometry = geometry),
    alpha = 0.6, color = "transparent"
  ) +
  # River pixel colours
  geom_sf(
    data = pts_res, aes(col = Y2015, geometry = geometry, size = upa),
    alpha = .9, stroke = 0, shape = 15
  ) +
  scale_size(range = c(0.08, 0.4), trans = "sqrt", guide = "none") +
  scale_fill_gradientn(
    colors = palet, breaks = br_map, limits = limi,
    trans = scales::modulus_trans(.3),
    oob = scales::squish, na.value = colNA, name = "Change (%)"
  ) +
  scale_color_gradientn(
  ...
  geom_sf(
    data = res_new_sf, aes(size = res, geometry = geometry),
    color = "black", fill = "orange", shape = 23, alpha = 0.5, stroke = 0.4
  ) +
  scale_size(
    range = c(0.1, 4), trans = "sqrt",
...
ggsave(paste0(plotDir, "/Fig3_mapF_reservoirs_", haz, "_F2.jpg"),
  fig_res,
  width = 23, height = 20, units = "cm", dpi = 800
)
...
# Land-use rasters live under the canonical data root (hydroDir/landuse).
rast_forest <- raster(file.path(hydroDir, "landuse/fracforest_ch20201951.tif"))
rast_sealed <- raster(file.path(hydroDir, "landuse/fracsealed_ch20201951.tif"))
rast_water <- raster(file.path(hydroDir, "landuse/fracwater_ch20201951.tif"))
...
fig_lu <- ggplot(basemap) +
  ...
  geom_sf(
    data = pag_lu, aes(fill = Rchange_rel.mean, geometry = geometry),
    alpha = 0.6, color = "transparent"
  ) +
  # River pixel colours
  geom_sf(
    data = pts_lu, aes(col = Y2015, geometry = geometry, size = upa),
    alpha = .9, stroke = 0, shape = 15
  ) +
  scale_size(range = c(0.08, 0.4), trans = "sqrt", guide = "none") +
  scale_fill_gradientn(
    colors = palet_clim, breaks = br_map, limits = limi,
    trans = scales::modulus_trans(.3),
...
fig_wu <- ggplot(basemap) +
  ...
  # HER background shading (RL change)
  geom_sf(
    data = pag_wu, aes(fill = Rchange_rel.mean, geometry = geometry),
    alpha = 0.6, color = "transparent"
  ) +
  # River pixel colours
  geom_sf(
    data = pts_wu, aes(col = Y2015, geometry = geometry, size = upa),
    alpha = .9, stroke = 0, shape = 15
  ) +
  scale_size(range = c(0.08, 0.4), trans = "sqrt", guide = "none") +
  scale_fill_gradientn(
  ...
  geom_sf(
    data = nuts3_3035, aes(color = sign), fill = NA, linewidth = .1,
...
paggC$driver <- "Clim" # climate
paggL <- Dchangelist[[4]]$PagD
paggL$driver <- "Lu" # land use
paggR <- Dchangelist[[3]]$PagD
paggR$driver <- "Res" # reservoirs
paggW <- Dchangelist[[5]]$PagD
paggW$driver <- "Wu" # water use
...
fig4 <- ggplot(basemap) +
  ...
  geom_sf(
    data = nutplot, aes(fill = factor(maxcol), geometry = geometry),
    color = "transparent", alpha = .6, size = 0.25, stroke = 0, shape = 15
  ) +
  geom_sf(
    data = points_pix, aes(col = factor(maxcol), geometry = geometry, size = upa),
    alpha = .9, stroke = 0, shape = 15
  ) +
  coord_sf(xlim = c(min(nco[, 1]), max(nco[, 1])), ylim = c(min(nco[, 2]), max(nco[, 2]))) +
...
if (RUN_SUPP) {
  # =============================================================
  #  Barplot – % of pixels with |<U+0394>RL| > 5%, ranked by driver
  # =============================================================
  ...
  sign_labs <- c("increase" = "<U+0394>RL > +5%", "decrease" = "<U+0394>RL < -5%")
  ...
  fig_bar <- ggplot(
    ...
    scale_y_continuous(
      name   = paste0("Share of river pixels with |<U+0394>", haz, " RL| > 5%  (%)"),
    ...
    ggtitle(paste0(
      "Share of river pixels with |<U+0394>", haz,
  ...
  if (haz == "Drought") {
    ...
    figS1 <- ggplot(basemap) +
      geom_sf(fill = "gray95", color = "gray10", size = 0.5) +
      geom_sf(
        data = pts_IR, aes(col = factor(gen_IR), geometry = geometry, size = upa),
        alpha = .9, stroke = 0, shape = 15
      ) +
      coord_sf(xlim = c(min(nco[, 1]), max(nco[, 1])), ylim = c(min(nco[, 2]), max(nco[, 2]))) +
      scale_colour_manual(
        values = colIR, name = "IR",
  ...
  for (yr in 1952:2020) {
    ...
    ParamSpec[merd, ]
    RlevErrtH <- rbind(c(
      mean(Rlev$returnLevels, na.rm = TRUE),
      mean(Rlev$returnLevelErr, na.rm = TRUE), yr
    ), RlevErrtH)
  }
  ...
  figS2 <- ggplot(RlevErrtH) +
    ...
    scale_y_continuous(
      name = "Change in 10yRL", breaks = seq(80, 120, 2),
      trans = scales::modulus_trans(.6)
    ) +
    scale_x_continuous(
      breaks = seq(1950, 2020, 5), labels = seq(1950, 2020, 5),
  ...
  X0 <- Paramf$nPeaks / 70
  RlevErrf <- calculate_return_levels(Paramf, X0 * 100, ci = 1)
  RlevErrf$year <- 2015
  RlevErri <- calculate_return_levels(paraU, ci = 1)
  RlevErri$year <- 1955
  ...
  figS3 <- ggplot(basemap) +
    ...
    geom_sf(
      data = Paraplot, aes(col = relErr, geometry = geometry, size = upa),
      alpha = 1, stroke = 0, shape = 15
    ) +
    scale_size(range = c(0.1, 0.5), trans = "sqrt", guide = "none") +
    coord_sf(xlim = c(min(nco[, 1]), max(nco[, 1])), ylim = c(min(nco[, 2]), max(nco[, 2]))) +
    scale_color_gradientn(
      colors = paletS, breaks = seq(0, 200, 20), limits = c(0, 200),
  ...
  figS4 <- ggplot(basemap) +
    geom_sf(fill = "gray90", color = "darkgrey", size = 0.5) +
    geom_sf(
      data = Paraplot, aes(col = S2n, geometry = geometry, size = upa),
      alpha = 1, stroke = 0, shape = 15
    ) +
    scale_size(range = c(0.1, 0.5), trans = "sqrt", guide = "none") +
    coord_sf(xlim = c(min(nco[, 1]), max(nco[, 1])), ylim = c(min(nco[, 2]), max(nco[, 2]))) +
    scale_color_manual(values = c(
      "Unstable" = "purple", "Err > Change" = "darkred",
      "Change > Err" = "royalblue"
    ), name = "Error classes", guide = guide_colorbar(
      direction = "horizontal",
      title.position = "top",
      barwidth = 10 # You can control the width of the bar here
    )) +
  ...
  ggsave(paste0(plotDir, "/FigS5_orderedHR_", haz, ".jpg"),
    figS5,
    width = 40, height = 8, units = "cm", dpi = 400
  )
  ...
  for (bn in bio_names) {
    ...
    figS6_bn <- ggplot(trtF_bn) +
      ...
      scale_y_continuous(
        name = "Mean change (% of 10Y RL)", breaks = br_bgr,
        trans = scales::modulus_trans(.6)
      ) +
      scale_x_continuous(
        breaks = xlabs_bgr, labels = xlabs_bgr, name = "Decades",
        minor_breaks = seq(1955, 2005, 10), expand = c(.01, .01)
      ) +
      scale_fill_manual(values = colorn, name = "Drivers", labels = clabels) +
      scale_color_manual(values = colorz, name = "Drivers", labels = clabels) +
  ...
  figS7 <- ggplot(UpaF) +
    ...
    geom_rect(
      aes(
        xmin = loc - .45, xmax = loc + .45, ymin = cq1, ymax = cq2,
        fill = factor(driver), group = factor(driver)
      ),
      alpha = 0.5, position = position_dodge(width = .9)
    ) +
    ...
    geom_rect(
      aes(
        xmin = loc - .45, xmax = loc + .45, ymin = med - 1e-1, ymax = med + 1e-1,
        fill = factor(driver), group = factor(driver)
      ),
      alpha = 1, position = position_dodge(width = .9)
    ) +
    scale_y_continuous(
      name = "Mean change (% of mean 10yRL)", breaks = br_upa,
      trans = scales::modulus_trans(.6)
    ) +
    scale_x_continuous(
      breaks = 1:5,
...
outfile <- if (haz == "Flood") {
  Output_fl_year <- list(
    TrendPix = Alltrend, TrendRegio = trendRegio,
    Out2020 = pointsAD, DataI = DataI
  )
  save(Output_fl_year, file = file.path(floodDir, "outputs_flood_year_relxHR_FINAL.Rdata"))
} else {
  Output_dr_nonfrost <- list(
    TrendPix = Alltrend, TrendRegio = trendRegio,
    Out2020 = pointsAD, DataI = DataI
  )
  save(Output_dr_nonfrost, file = file.path(droughtDir, "outputs_drought_nonfrost_relxHR_FINAL.Rdata"))
}

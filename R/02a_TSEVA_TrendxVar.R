...
# Resolve script directory (works with Rscript on HPC and interactive R)
...
source("config_paths.R") # provides hydroDir, geoDir, threshDir, ID_MULT, ...

# Arguments importation #############################
# HPC usage: Rscript 02a_TSEVA_TrendxVar.R <Nsq> <haz> <sce>
args <- commandArgs(TRUE)
if (length(args) >= 1) {
  Nsq <- as.numeric(args[1])
  haz <- ifelse(length(args) >= 2, args[2], "drought")
  sce <- ifelse(length(args) >= 3, args[3], "Histo")
} else {
  Nsq <- 42
  haz <- "drought"
  sce <- "Histo"
...
tsEvaTransformSeriesToStationaryPeakTrendX <- function(timeStamps, series, timeWindow, xid) {
  ...
  detrend_result <- tsEvaDetrendTimeSeries(
    timeStamps, series_above_threshold,
    timeWindow
  )
  ...
  variance_series <- tsEvaNanRunningVariance(
    detrended_series_above_threshold,
    n_run_mean
  )
  ...
  running_moments <- tsEvaNanRunningStatistics(
    stationary_series_no_na,
    n_run_mean
  )
  stat_ser_3_mom <- tsEvaNanRunningMean(
    running_moments$rn3mom,
    ceiling(n_run_mean)
  )
  stat_ser_4_mom <- tsEvaNanRunningMean(
    running_moments$rn4mom,
    ceiling(n_run_mean)
  )
...
# Thresholds produced by 01_TSEVA_TrendThresholdSel.R (same ID_MULT scheme,
# so no id remapping is needed anymore).
TH <- read.csv(file.path(threshDir, paste0("trenTH_SCF_", tail, "_", Nsq, ".csv")))
...
xtrempoints <- read.csv(file.path(threshDir, paste0("xtrempoints_SCF_", tail, "_", Nsq, ".csv")))
...
for (idfix in startid:endid) {
  ...
  thresh <- thresh$th
  frosttime <- NA
  df.disX <- disNcopenloc(filename, riverDir, outhybas, idfix)
...
save(Results, file = file.path(trendVarDir, paste0("TrendVarX_", outlets, haz, "_", Nsq, "_1951_2020.Rdata")))
